...
function PANEL:Init()
    ...
    self.bg.Paint = function(s, w, h)
        ...
        if self.type == 0 then
            draw.SimpleText(tostring(ASEEEM_PS.func.GetNW(LocalPlayer(), 'point', ASEEEM_PS.enums.NWType.INT, language.GetPhrase('ASEEEM_PS_unknown'))), 'Roboto24', w/2, h/2, ASEEEM_PS.theme.btnTextColor, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
        elseif self.type == 1 then
            draw.SimpleText(tostring(ASEEEM_PS.func.GetNW(LocalPlayer(), 'proPoint', ASEEEM_PS.enums.NWType.INT, language.GetPhrase('ASEEEM_PS_unknown'))), 'Roboto24', w/2, h/2, ASEEEM_PS.theme.btnTextColor, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
        elseif self.type == -1 then
